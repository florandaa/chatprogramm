...
def discovery_loop():
    ...
    while True:
        data, addr = socket1.recvfrom(1024) # data=Inhalt, addr=(IP,Port des Absenders)-Empfangen von Daten (max. 1024 Bytes)
        message = data.decode('utf-8').strip()# Dekodieren der empfangenen Daten von Bytes in Text/String. #strip() entfernt führende und nachfolgende Leerzeichen
        parts = message.split() #Zerlegen der Nachricht in Teile, getrennt durch Leerzeichen

        if not parts:
            continue ##Wenn die Nachricht leer ist, überspringe den Rest der Schleife
        command = parts[0]# Liest den ersten Teil der Nachricht als Befehl(zb. JOIN,WHO,LEAVe)

        if command == "JOIN" and len(parts) == 3:# Überprüfen, ob der Befehl "JOIN" ist. len(parts) == 3 stellt sicher, dass genau 3 Teile vorhanden sind
            handle = parts[1] #NAme
            ...
            entries = [f"{h} {ip} {p}" for h, (ip, p) in participants.items()] # Erstellen einer Liste von Einträgen "Handle IP Port"
            response = "KNOWUSERS " + ", ".join(entries) # Zusammenfügen der Einträge zu einer Antwort
            socket1.sendto(response.encode('utf-8'), (addr[0], 5000)) # Senden der Antwort an die IP-Adresse des Absenders 

        elif command == "LEAVE" and len(parts) == 2: 
            handle = parts[1] #Name des Teilnehmers, der den Raum verlassen möchte
            if handle in participants:# Überprüft, ob der Teilnehmer im Dictionary vorhanden ist
...
threading.Thread(target=discovery_loop, daemon=True).start() #Loop damit es im Hintergrund läuft
